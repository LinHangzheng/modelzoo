...
class TransformerEncoder(BaseLayer):
    ...
    def call(self, input, mask=None, ret_scores=False):
       #print("encoder input: ", input)
        norm_input = self.ln1(input)
       #print("first norm out: ", norm_input)
        if ret_scores:
            msa, scores = self.MSA(norm_input, mask, ret_scores)
        else:
            msa = self.MSA(norm_input, mask)
       #print("MSA out: ", msa)
        x = msa + norm_input
        norm_msa = self.ln2(x)
...
class MSA(BaseLayer):
    ...
    def call(self, input, mask=None, ret_scores=False):
        return self.mha(input, input, mask=mask)
